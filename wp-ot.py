	...
	try:
		...
		if cek == 'error':
			...
			if sys.argv[1] in exp:
				print '%s[ %ssuccess %s] %s/wp-content/uploads/%s/%s/%s'%(W0,G0,W0,target,now.strftime('%Y'),now.strftime('%m'),sys.argv[1])
				open('success.txt','a+').write(target+'/wp-content/uploads/'+now.strftime('%Y')+'/'+now.strftime('%m')+'/'+sys.argv[1]+'\n')
			else:
